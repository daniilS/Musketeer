...
class ComplexSpeciationMixin:
    ...
    def complexFormsBinaryComplex(self):
        ...
        return np.array(
            [
                [
                    np.sum(eye[[row, col]], axis=0).tolist() in M
    ...
    @property
    def complexMaxTotalValency(self):
        # output[i] = the max number of guests that can simultaneously bind to one i
        output = np.zeros(self.freeCount, dtype=int)
        for row in self.complexStoichiometries:
            pass
            # TODO: implement
